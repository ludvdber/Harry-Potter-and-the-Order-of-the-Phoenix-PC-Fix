...
| **FXAA + adaptive sharpening** | on | Single `ps_3_0` post-process pass that does luma-based edge AA and sharpens flat areas. Near-zero perf cost. Also hosts color grading and SSAO so it's one shader, one draw call. |
...
| **Screen-space ambient occlusion** | off | 8-tap radial depth comparison folded into the FXAA shader. Auto depth-stencil is overridden to INTZ (FOURCC, every D3D10+ GPU) so depth becomes sampleable, and `CreateDepthStencilSurface` is intercepted so game-created depth gets the same treatment. Per-pixel depth thresholds are perspective-scaled so a fixed delta works at all distances. |
| **Color grading folded into the FXAA pass** | on, subtle | ASC-CDL Lift / Gain → Gamma → Vibrance → Vignette, sharing the FXAA shader so it costs zero extra draw call. Defaults give HP5's flat 2007 look a subtle modern pop without making it look like a different game. |
...
- **`FXAA`** (default `1`) — post-process AA + adaptive sharpening. Also gates color grading and SSAO — turning FXAA off disables both. Near-zero cost.
...
The 2007 base render is technically clean but visually flat. The wrapper applies a subtle ASC-CDL grade in the same shader as FXAA — no extra draw call.
...
Off by default — opt-in because it requires an INTZ-capable GPU (every D3D10+ card supports it, falls back silently otherwise). When on, the wrapper overrides the auto depth-stencil format and intercepts `CreateDepthStencilSurface` so it can sample scene depth, then does an 8-tap radial comparison around each pixel in the FXAA shader.

- **`SSAO`** (default `0`) — `0` off, `1` on. Adds ~8 depth taps per pixel; negligible cost on any GPU made in the last decade.
...
- **`Width` / `Height`** (default `0`) — `0` = use the game's default resolution (typically your screen's native). Override only if you want a different render resolution. In borderless fullscreen the result is stretched to your screen, so picking a value lower than native produces a blurry upscale.

### `[MAIN]` and `[LAUNCHER]`

These are upstream sections kept as-is — see comments in `d3d9.ini` for `FPSLimit`, `FullScreenRefreshRateInHz`, `DisplayFPSCounter`, `ForceWindowedMode`, and the launcher app-injection options.
...
- **This fork's enhancements** (MSAA cascade, AF/LOD forcing, mipmap regeneration, FXAA + sharpening, color grading, SSAO, INTZ pipeline, frame latency cap, DirectInput8 proxy + Alt+Tab mouse re-Acquire): added in this fork.